...
describe('useFavoriteToggle', () => {
  ...
  it('should toggle favorite status successfully', async () => {
    ...
    await waitFor(async () => {
      expect(await result.current.toggleFavorite(1, true)).toEqual(
        mockResponseData,
      )
    })
  ...
  it('should return an error message if the toggle request fails', async () => {
    ...
    await waitFor(async () => {
      expect(await result.current.toggleFavorite(1, true)).toStrictEqual([
        'Note not found',
      ])
    })
  ...
  it('should return errors received from API if the toggle request fails', async () => {
    fetch.mockResolvedValue({
      ok: false,
      json: async () => ({
        error: ['Title is null', 'Description is greater than allowed'],
      }),
    })
  ...
  it('should throw an error if the fetch call fails', async () => {
    ...
    await expect(result.current.toggleFavorite(1, true)).rejects.toThrow(
      'Network error',
    )
