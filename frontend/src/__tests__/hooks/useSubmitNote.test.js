...
describe('useSubmitNote', () => {
  ...
  it('should create notes successfully', async () => {
    const mockResponseData = { title: 'Test', description: 'Description', favorite: false };
    ...
    await waitFor(async () => {
      expect(
        await result.current.submitNote(mockResponseData)
  ...
  it('should return an error message if the create request fails', async () => {
    ...
    await expect(result.current.submitNote(mockResponseData))
      .rejects.toThrow('Failed to submit the note');
  ...
  it('should return errors received from API if the create request fails', async () => {
    const mockResponseData = { title: '', description: 'Description', favorite: true };
    fetch.mockResolvedValue({
      ok: false,
      json: async () => ({ errors: ['Title is null', 'Description is greater than allowed'] })
    });

    const { result } = renderHook(() => useSubmitNote());

    await waitFor(async () => {
      expect(await result.current.submitNote(mockResponseData))
        .toStrictEqual({ errors: ['Title is null', 'Description is greater than allowed'] });
    });
