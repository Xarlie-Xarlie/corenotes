...
describe('useFetchNotes', () => {
  ...
  it('should fetch notes successfully without searchTerm', async () => {
    const mockNotes = [{ id: 1, title: 'Test Note', description: 'Description' }];
  ...
  it('should fetch notes successfully with searchTerm', async () => {
    const mockNotes = [{ id: 2, title: 'Search Note', description: 'Description' }];
    fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ notes: mockNotes }),
    });
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('http://localhost:3333/api/notes?search=Search');
      expect(result.current.notes).toEqual(mockNotes);
      expect(result.current.error).toBeNull();
    })
  });
