...
describe('App Component', () => {
  ...
  beforeEach(() => {
    ...
    useFetchNotes.mockReturnValue({
      notes: [], setNotes: jest.fn(), error: null
    });
  })
...
describe('App Component Functions', () => {
  ...
  test('handleNoteCreated updates notes state', async () => {
    const newNote = { id: 1, title: 'Test Title', description: 'Test Description', favorite: false };
    ...
    await waitFor(() => {
      expect(mockSubmitNote).toHaveBeenCalledWith({ title: 'Test Title', description: 'Test Description', favorite: false })
  ...
  test('handleNoteDeleted updates notes state', async () => {
    ...
    const noteToDelete = { id: 1, title: 'Note 1', description: 'Description 1', favorite: true };
  ...
  test('handleUpdatedNote updates notes state', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockUpdateNote).toHaveBeenCalledWith(1, 'Updated Note Title', 'Updated description', 'color');
      expect(setNotes).toHaveBeenCalledWith(expect.any(Function));
      expect(setNotes).toHaveBeenCalledWith(expect.arrayContaining([]));
  ...
  test('displays error toast when fetch notes fails', async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Something went wrong while fetching notes.');
    })
  });
});
